...
author: "A Pathmeswaran<br>pathmes@kln.ac.lk<br>2025-03-03"
format: 
  revealjs:
...
## Data from Kirkwoods book {visibility="hidden"}
...
## MF postivity by area {visibility="hidden"}
...
## Table with OR {visibility="hidden"}
...
## MF postivity by area 2 {visibility="hidden"}
...
## Logistic regression output {visibility="hidden"}
...
print(paste("Odds of LBW =", round(oddsH140, 3)))
Pr140 <- oddsH140/ (1 + oddsH140)
print(paste("Probability of LBW =", round(Pr140, 2)))
```
...
as_tibble(pt) |>
  kableExtra::kable(digits = c(1, 3, 2))
```
...
cbind(beta = coef(m5), confint(m5))
exp(cbind(OR = coef(m5), confint(m5)))
```
